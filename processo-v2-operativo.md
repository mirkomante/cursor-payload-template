...
- [x] Formalizzata l'Azione 4 (formato obbligatorio dei file di fase, `depends_on`), deferita il 2026-08-29. Trigger: bug di processo osservato su un progetto reale (`vma_vd-backend-backoffice`) — in `fase-2-login.md` l'ordine numerico non rifletteva l'ordine di esecuzione reale (2.2 andava fatta prima di 2.8 per il guardrail anti-lista-vuota; 2.7 andava fatta insieme a 2.8, prima di 2.4, che nasconde il form locale standard), causando una finestra priva di qualunque via di accesso Admin funzionante. Formalizzazione: non a livello di frontmatter YAML del file (resta `stato: bozza | validato`), ma un nuovo campo testuale obbligatorio `**Dipende da**:` ad ogni sottofase, stesso stile di `**Stato**`/`**Obiettivo**` — deliberatamente distinto dal `depends_on` del DAG di progetto in `piano.yaml` (§2), che resta a grana di fase, non di sottofase. Contestualmente corretto anche il contenuto di `fase-2-login.md`: nota parziale sull'ordine sostituita da una sezione "Ordine di esecuzione consigliato" completa (sul modello di `fase-3-deploy.md`), più due item di merito (2.2: popolare l'allow-list con l'identità reale subito, non a 2.10; 2.8: l'email del seed dev'essere un account realmente autenticabile col provider SSO scelto). Applicato per ora solo a `fase-2-login.md` (10/10 sottofasi) — non retroattivo su `fase-1-setup.md`/`fase-3-deploy.md`/varianti, per proporzionalità: si applicherà quando quei file verranno toccati per altro motivo. Nessun controllo meccanico nuovo (`check-depends`): un solo caso non giustifica un tool dedicato — stesso criterio di riapertura di §4.4. *(fatto 2026-09-20)*
