...
- [ ] **Provider email (variante)**: il dominio mittente è verificato presso il provider scelto (per Resend: stato "Verified" in dashboard — vedi `email/01a-resend.mdc` e `fase-2-email-resend.md`). Se il dominio definitivo del progetto non è ancora confermato a questo punto, usare temporaneamente un dominio di sviluppo/placeholder già posseduto (non il sandbox del provider, che non invia a utenti reali arbitrari) — annotando esplicitamente che andrà sostituito prima del go-live (vedi nota sotto).

**Attenzione per il futuro** (solo se in questa sottofase è stato usato un dominio provvisorio per la verifica email): prima del go-live definitivo, ripetere la verifica dominio presso il provider email con il dominio reale del progetto (nuovi record DNS, nuova propagazione) e aggiornare `RESEND_FROM_ADDRESS`/`RESEND_FROM_NAME` (o equivalenti) di conseguenza — stesso pattern del rimando auth descritto in § 3.3 per il dominio personalizzato dell'app.
