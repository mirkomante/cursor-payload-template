...
fase-1-db-postgres.md             variante database
...
fase-3-db-postgres.md             variante database
...
| 2    | Email             | Resend           | `fase-2-email-resend.md`      | ✅ pronta                     | 2026-09                           |
...
- **Aggiunta regola di range numerico e di provenienza per le ADR di catalogo**, in `00-come-eseguire-il-piano.md` (Passo 0): risolve una collisione di numerazione osservata su un progetto reale — un'ADR di progetto numerata come una di catalogo (`ADR-002`) mascherava silenziosamente il rimando corretto per un agente che cercasse il file nel repo di progetto invece che nel catalogo. Range assegnato: le ADR di catalogo restano in `ADR-000`–`ADR-099`, i progetti numerano le proprie a partire da `ADR-100` (rollover a `ADR-200`, `ADR-300`, ... per famiglie successive). Aggiunta anche la regola che ogni rimando a un'ADR di catalogo in un file di fase riporti esplicitamente la provenienza, non solo il nome del file — applicata subito alle 3 citazioni esistenti in `fase-2-login.md` (§2.1, 2.4, 2.5, 2.6). Nessun rename delle ADR-001/002/003 stesse. *(2026-09-13)*
- **Scelta esplicita del dominio email spostata a Fase 2 §2.6**, con checkpoint di chiusura in Fase 3 §3.2: emerso dallo stesso gap di deliverability aggiunto in `cursor-rules` (`email/01-email-invarianti.mdc`/`01a-resend.mdc`). `fase-2-email-resend.md` §2.6 offre ora una scelta non vincolante tra restare sul sandbox Resend (rimandando la verifica dominio a Fase 3) o verificare già un sottodominio dedicato — dev o del progetto, consigliato per validare la deliverability reale in anticipo. `fase-3-deploy.md` §3.2 richiama la scelta fatta a §2.6 invece di riproporla come nuova; chiarito anche che il rimando esplicito "dominio personalizzato/DNS avanzato" fuori scope Fase 3 riguarda l'hosting dell'app, non il dominio per l'invio email. *(2026-09-20)*
